...
def compute_flow(model, video):
    # video: [B*T, C, H, W], scale: [0, 255]
    flow = []
    for image1, image2 in zip(video[:-1], video[1:]):
        image1 = image1.unsqueeze(0) # [1, C, H, W]
        image2 = image2.unsqueeze(0) # [1, C, H, W]
        flow_low, flow_up = model(image1, image2, iters=20, test_mode=True)
        flow.append(flow_up)
    flow = torch.stack(flow, dim=0) # [T-1, 2, H, W]
    return flow
    
    
def compute_optical_loss(model, samples, videos):
    # samples & videos: [B*T, C, H, W], scale: [0, 255]
    samples_flow = compute_flow(model, samples)
    videos_flow = compute_flow(model, videos)
    loss = (samples_flow - videos_flow).abs().mean()
    return loss
...
def main():
    ...
    # == build RAFT model ==
    raft_args = argparse.Namespace(model="models/raft-things.pth", small=False, mixed_precision=True, alternate_corr=False)
    ...
    for epoch in range(start_epoch, cfg_epochs):
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                samples = samples.reshape(-1, *samples.shape[2:])  # [B, T, C, H, W] -> [B*T, C, H, W]
                videos = videos.reshape(-1, *videos.shape[2:])  # [B, T, C, H, W] -> [B*T, C, H, W]
                # 从[-1, 1] -> [0, 255]
                videos = (videos + 1) * 127.5
                loss = compute_optical_loss(raft_model, samples, videos)
                # == backward & update ==
