...
class VideoTextDataset(torch.utils.data.Dataset):
    ...
    def get_type(self, path):
        ...
        if os.path.isdir(path):
            return "dir"
        elif ext.lower() in VID_EXTENSIONS:
            return "video"
        else:
    ...
    def getitem(self, index):
        ...
        if file_type == "dir":
            vframes = []
            for i in range(32):
                img = pil_loader(os.path.join(path, f"rgba_{i:05d}.jpg"))
                img = np.array(img)
                vframes.append(img)
            vframes = np.stack(vframes)
            video = torch.from_numpy(vframes)  # [T, H, W, C=3]
        ...
        ret = {
            "video": video, 
            "fps": self.fps if self.fps is not None else video_fps,
...
@DATASETS.register_module()
class VideoTextMaskDataset(torch.utils.data.Dataset):
    """load video according to the csv file.
    ...
    def getitem(self, index):
        ...
        if file_type == "dir":
            ...
            video = transform(video)  # T C H W
            # TODO
            mask = np.load(os.path.join(path, "mask.npy")) # [T, n, H, W]
            mask = torch.from_numpy(mask)
        else:
            raise NotImplementedError("Only support dir data type.")
